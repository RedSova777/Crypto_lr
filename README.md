...
Скрипт реализует ручной разбор формата закрытых ключей OpenSSH `openssh-key-v1` без использования специализированных библиотек для парсинга ключей. Он последовательно проверяет текстовую обёртку, декодирует Base64, разбирает бинарные SSH-структуры, определяет тип ключа, извлекает параметры RSA/ECDSA/Ed25519 и выводит результат в удобном формате.
